...
/// ```
pub fn anycase<'a>(literal: &'static str) -> impl Parser<'a, &'a str, &'a str> {
	move |input: &'a str| {
...
pub fn one_of0<'a>(
	chars: &[char],
) -> impl Parser<'a, &'a str, &'a str> + use<'_, 'a> {
...
pub fn one_of_char<'a>(
	chars: &[char],
) -> impl Parser<'a, &'a str, char> + use<'_, 'a> {
...
macro_rules! impl_parse_uint {
	($name:ident, $type:ty) => {
		...
		pub fn $name<'a>() -> impl Parser<'a, &'a str, ($type, &'a str)>
		{
			|input: &'a str| {
